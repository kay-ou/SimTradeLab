...
def handle_data(context, data):
    ...
    try:
        ...
        if position:
            log.info(
                f"当前持仓: {position['amount']}股, 成本价: {position['cost_basis']:.2f}, 市值: {position['market_value']:.2f}"
            )
        else:
            log.info("当前无持仓")
        ...
        if g.trade_count == 0:
            ...
            if order_id:
                ...
                log.info(
                    f"订单状态: {order_info['status']}, 成交数量: {order_info['filled_amount']}"
                )
        ...
        if final_open_orders:
            ...
            for order_id, order_info in final_open_orders.items():
                log.info(
                    f"  订单{order_id[:8]}: {order_info['order_type']}单, "
