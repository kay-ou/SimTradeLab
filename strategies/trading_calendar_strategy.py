...
def handle_data(context, data):
    ...
    try:
        ...
        if g.last_trade_day is None:
            ...
            # 计算距离上次交易的交易日数量
            days_since_last_trade = _count_trading_days_between(
...
def before_trading_start(context, data):
    ...
    if (
        ...
        and current_date.date() == month_first_trading_day.date()
    ):
        log.info("今日是本月第一个交易日")

        # 获取本月所有交易日
        month_end = (current_date.replace(day=28) + pd.Timedelta(days=4)).replace(
            day=1
        ) - pd.Timedelta(days=1)
...
def after_trading_end(context, data):
    ...
    if positions:
        for security, position in positions.items():
            if position and position["amount"] > 0:
                log.info(
                    f"持仓 {security}: {position['amount']}股, "
    ...
    if not next_trading_day or next_trading_day.month != current_date.month:
        ...
        # 月末总结
        month_start = current_date.replace(day=1)
        month_trading_days = get_trade_days(
            start_date=month_start, end_date=current_date
        )
        log.info(f"本月实际交易日数量: {len(month_trading_days)}")

        if g.last_trade_day:
            days_since_last = _count_trading_days_between(
                g.last_trade_day, current_date
            )
