...
def initialize(context):
    ...
    log.info(
        f"RSI策略初始化完成 - 周期:{g.rsi_period}, 超卖线:{g.rsi_oversold}, 超买线:{g.rsi_overbought}"
    )
...
def handle_data(context, data):
    ...
    if buy_signal:
        ...
        log.info(
            f"RSI超卖买入 - RSI:{current_rsi:.2f} < {g.rsi_oversold}, 买入价值:{target_value:.0f}"
        )
    ...
    if current_position > 0:
        ...
        log.info(
            f"持仓状态 - RSI:{current_rsi:.2f}, 持股:{current_position}, 市值:{position_value:.0f}"
        )
    else:
...
def after_trading_end(context, data):
    ...
    if hasattr(context.portfolio, "starting_cash"):
        ...
        log.info(
            f"日终总结 - 总资产:{total_value:.2f}, 现金:{cash:.2f}, 收益率:{total_return:.2f}%"
        )
