...
def handle_data(context, data):
    ...
    try:
        ...
        rsi_value = (
            rsi_data[(f"RSI{g.rsi_period}", security)].iloc[-1]
        ...
        cci_value = (
            cci_data[(f"CCI{g.cci_period}", security)].iloc[-1]
        ...
        # 记录技术指标值
        log.info(
            f"技术指标 - MACD_DIF: {macd_dif:.4f}, MACD_DEA: {macd_dea:.4f}, MACD_HIST: {macd_hist:.4f}"
        )
        log.info(
            f"技术指标 - RSI: {rsi_value:.2f}, KDJ_K: {kdj_k:.2f}, KDJ_D: {kdj_d:.2f}, KDJ_J: {kdj_j:.2f}"
        )
        ...
        if (
            buy_signals >= 2
            and current_shares
            < g.max_position_ratio * context.portfolio.total_value / current_price
        ):
            # 买入信号
