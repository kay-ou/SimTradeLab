...
def make_trading_decision(context, data, stock):
    ...
    if buy_signal:
        ...
        if (
            target_amount > 0
            and context.portfolio.cash >= target_amount * current_price
        ):
...
def after_trading_end(context, data):
    ...
    if positions:
        ...
        for stock, pos in positions.items():
            ...
            pnl_pct = (
                (pnl / (pos.amount * pos.cost_basis)) * 100 if pos.amount > 0 else 0
            )
            log.info(
    ...
    for stock in g.stocks:  # 修复这里，迭代单个股票
        if stock in g.price_history and len(g.price_history[stock]) >= g.long_window:
            if stock in g.last_prices:
                ...
                log.info(
                    f"  {stock}: 价格{current_price:.2f}, MA{g.short_window}={short_ma:.2f}, "
