...
def handle_data(context, data):
    ...
    try:
        ...
        prices = (
            hist_data[security]
            if security in hist_data.columns
            else hist_data.iloc[:, 0]
        )
        ...
        # 计算前一日均线（用于判断交叉）
        ma_short_prev = (
            prices.rolling(window=g.short_window).mean().iloc[-2]
            if len(prices) > g.short_window
            else ma_short
        )
        ma_long_prev = (
            prices.rolling(window=g.long_window).mean().iloc[-2]
            if len(prices) > g.long_window
            else ma_long
        )

        log.info(f"当前价格: {current_price:.2f}")
        log.info(
            f"MA{g.short_window}: {ma_short:.2f}, MA{g.long_window}: {ma_long:.2f}"
        )
        ...
        if golden_cross and current_shares == 0:
            # 金叉买入信号
            max_shares = (
                int(context.portfolio.cash * g.position_ratio / current_price / 100)
                * 100
            )
            if max_shares > 0:
...
def after_trading_end(context, data):
    ...
    if position and position["amount"] > 0:
        log.info(
            f"持仓 {g.security}: {position['amount']}股, "
