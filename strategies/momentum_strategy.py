...
def handle_data(context, data):
    ...
    try:
        ...
        hist_data = get_history(
            max(g.momentum_period, g.volume_period) + 5,
            "1d",
            ["close", "volume"],
            security,
        )
...
def _calculate_momentum(hist_data, security):
    """计算价格动量"""
    prices = (
        hist_data["close"][security]
        if security in hist_data["close"].columns
        else hist_data["close"].iloc[:, 0]
    )
...
def _calculate_volume_ratio(hist_data, security, current_volume):
    """计算成交量比率"""
    volumes = (
        hist_data["volume"][security]
        if security in hist_data["volume"].columns
        else hist_data["volume"].iloc[:, 0]
    )
...
def _check_entry_signals(context, current_price, momentum, volume_ratio):
    ...
    if momentum > g.momentum_threshold and volume_ratio > 1.2:
        max_shares = (
            int(context.portfolio.cash * g.position_ratio / current_price / 100) * 100
        )
        if max_shares > 0:
...
def _check_exit_signals(context, current_price, momentum, current_shares):
    ...
    if g.entry_price and current_price < g.entry_price * (1 - g.stop_loss_ratio):
        ...
        if order_id:
            ...
            log.info(
                f"🛑 止损卖出: 价格{g.entry_price:.2f} -> {current_price:.2f}, 亏损{loss_ratio:.2%}"
            )
    ...
    if g.trend_direction == "up" and momentum < -g.momentum_threshold / 2:
        ...
        if order_id:
            profit_ratio = (
                (current_price - g.entry_price) / g.entry_price if g.entry_price else 0
            )
