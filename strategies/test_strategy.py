...
def before_trading_start(context, data):
    ...
    if g.stocks_to_test:
        ...
        income_data = get_income_statement(
            g.stocks_to_test, fields=["revenue", "net_income", "eps_basic"]
        )
        log.info(f"损益表数据: {income_data}")

        # 测试资产负债表数据
        balance_data = get_balance_sheet(
            g.stocks_to_test,
        ...
        # 测试现金流量表数据
        cashflow_data = get_cash_flow(
            g.stocks_to_test, fields=["operating_cash_flow", "free_cash_flow"]
        )
        log.info(f"现金流量表数据: {cashflow_data}")

        # 测试财务比率数据
        ratios_data = get_financial_ratios(
            g.stocks_to_test, fields=["current_ratio", "roe", "debt_to_equity"]
        )
        log.info(f"财务比率数据: {ratios_data}")

        # 测试扩展的基本面数据（更多字段）
        extended_fundamentals = get_fundamentals(
            g.stocks_to_test, "income", fields=["revenue", "net_income", "roe", "roa"]
        )
...
def after_trading_end(context, data):
    ...
    for stock, position in context.portfolio.positions.items():
        if position.amount > 0:
            ...
            log.info(
                f"持仓 {stock}: {position.amount}股, 成本价: {position.cost_basis:.2f}, "
