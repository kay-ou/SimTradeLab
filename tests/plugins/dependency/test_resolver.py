...
class TestDependencyResolver:
    ...
    def test_resolve_dependencies_simple(
        self, resolver, mock_registry, sample_manifests
    ):
        """测试简单依赖解析"""
        # 设置注册表返回值
    ...
    def test_resolve_dependencies_multiple_targets(
        self, resolver, mock_registry, sample_manifests
    ):
        """测试多目标插件解析"""
        mock_registry.has_plugin.return_value = True
    ...
    def test_circular_dependency_error(self, resolver, mock_registry):
        ...
        mock_registry.has_plugin.return_value = True
        mock_registry.get_manifest.side_effect = lambda name: circular_manifests.get(
            name
        )
    ...
    def test_check_compatibility_with_conflicts(self, resolver, sample_manifests):
        ...
        assert any(
            "冲突" in error for error in compatibility_report["conflicting_plugin"]
        )
    ...
    def test_get_dependency_tree_max_depth(
        self, resolver, mock_registry, sample_manifests
    ):
        """测试依赖树最大深度限制"""
        mock_registry.get_manifest.side_effect = lambda name: sample_manifests.get(name)
