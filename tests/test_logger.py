...
class TestLogger:
    ...
    def test_trading_strategy_execution_logging(self, logger, capsys):
        ...
        # 记录策略决策过程
        logger.info(
            f"Strategy Decision: {strategy_name} generated {signal} signal for {symbol}"
        )
        logger.info(
            f"Signal Details: confidence={confidence}, position_size={position_size}"
        )
        logger.warning(
            f"Risk Alert: Large position size detected for {symbol}: {position_size} shares"
        )
    ...
    def test_risk_management_logging(self, logger, capsys):
        ...
        logger.critical(
            f"Emergency: Position concentration {position_concentration:.2%} exceeds limit"
        )
    ...
    def test_portfolio_performance_logging(self, logger, capsys):
        ...
        # 记录组合绩效
        logger.info(
            f"Portfolio Performance: Daily return {daily_return:.2%}, Cumulative {cumulative_return:.2%}"
        )
        logger.info(
            f"Risk Metrics: Sharpe ratio {sharpe_ratio:.2f}, Total value ${total_value:,}"
        )
