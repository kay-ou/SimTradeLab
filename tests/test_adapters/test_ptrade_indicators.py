...
class TestPTradeIndicators:
    ...
    def test_api_get_macd(self, adapter, sample_data):
        ...
        np.testing.assert_array_almost_equal(
            result["MACD_hist"].dropna(), hist_calculated.dropna(), decimal=10
        )
    ...
    def test_api_get_kdj(self, adapter, sample_data):
        ...
        np.testing.assert_array_almost_equal(
            result["J"].dropna(), j_calculated.dropna(), decimal=10
        )
    ...
    def test_api_check_limit(self, adapter):
        ...
        # 模拟当前数据
        mock_current_data = {
            "000001.SZ": {"last_price": 11.00, "pre_close": 10.00},
            "688001.SH": {"last_price": 24.00, "pre_close": 20.00},
            "ST000001.SZ": {"last_price": 10.50, "pre_close": 10.00},
        }

        with patch.object(
            adapter, "_api_get_current_data", return_value=mock_current_data
        ):
            # 测试普通股票涨停
    ...
    def test_check_limit_zero_pre_close(self, adapter):
        """测试昨收价为0的情况"""
        mock_current_data = {"000001.SZ": {"last_price": 10.00, "pre_close": 0.00}}
