...
class TestPTradeContext:
    ...
    def test_context_with_positions(self, context):
        """测试包含持仓的上下文"""
        # 添加一些持仓到组合
        position = Position(security="000001.XSHE", amount=1000, cost_basis=10.0)
        context.portfolio.positions["000001.XSHE"] = position

        # 验证可以通过上下文访问持仓
        assert "000001.XSHE" in context.portfolio.positions
        assert context.portfolio.positions["000001.XSHE"].amount == 1000
    ...
    def test_context_method_delegation(self, context):
        """测试上下文方法委托"""
        # 上下文应该可以访问组合和订单记录器的方法
        # 通过属性访问
        assert hasattr(context.portfolio, "add_position")
        assert hasattr(context.blotter, "add_order")
    ...
    def test_context_with_none_values(self):
        ...
        # None值应该被转换为合适的默认值
        assert context.universe == []  # None应该转为空列表
        assert context.benchmark is None  # benchmark可以是None
