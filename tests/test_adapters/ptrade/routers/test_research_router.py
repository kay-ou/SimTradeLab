...
class TestResearchAPIRouter:
    ...
    def test_trading_operations_not_supported(self, router):
        ...
        with pytest.raises(NotImplementedError, match="Trading operations are not supported"):
    ...
    def test_order_queries_not_supported(self, router):
        ...
        with pytest.raises(NotImplementedError, match="Order queries are not supported"):
    ...
    def test_position_queries_not_supported(self, router):
        ...
        with pytest.raises(NotImplementedError, match="Position queries are not supported"):
    ...
    def test_trade_queries_not_supported(self, router):
        ...
        with pytest.raises(NotImplementedError, match="Trade queries are not supported"):
            router.get_trades()

    def test_get_history(self, router):
        """测试获取历史数据"""
        # 测试DataFrame格式 - 研究模式返回空数据
        history = router.get_history(count=10, field=["open", "high", "low", "close", "volume"])
        
        assert isinstance(history, pd.DataFrame)
        assert history.empty  # 研究模式当前返回空DataFrame
    ...
    def test_get_price(self, router):
        ...
        assert price.empty  # 研究模式当前返回空DataFrame
    ...
    def test_research_mode_characteristics(self, router):
        ...
        trading_apis = [
            ...
            "order_market", "cancel_order", "cancel_order_ex",
