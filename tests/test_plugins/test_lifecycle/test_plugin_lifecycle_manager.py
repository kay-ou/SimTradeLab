...
from src.simtradelab.plugins.dependency.manifest import PluginManifest, PluginCategory
...
class TestPluginLifecycleManager:
    ...
    def test_lifecycle_hooks(self, lifecycle_manager):
        ...
        # 执行钩子
        plugin_info = PluginInfo(PluginManifest(
            name="test", version="1.0.0", description="Test", 
            author="Test", category=PluginCategory.UTILITY
        ))
...
class TestPluginLifecycleOperations:
    ...
    def test_load_plugin_with_dependencies(self, lifecycle_manager, plugin_with_deps):
        ...
        with patch.object(lifecycle_manager, '_import_plugin') as mock_import:
            ...
            assert result is True
            assert lifecycle_manager.get_plugin_state("base_plugin") == PluginState.LOADED
            assert lifecycle_manager.get_plugin_state("dependent_plugin") == PluginState.LOADED
    ...
    def test_load_plugin_force(self, lifecycle_manager):
        ...
        with patch.object(lifecycle_manager, '_import_plugin') as mock_import:
            ...
            assert result is True
            assert lifecycle_manager.get_plugin_state("test_plugin") == PluginState.LOADED
    ...
    def test_batch_operations(self, lifecycle_manager):
        ...
        with patch.object(lifecycle_manager, '_import_plugin') as mock_import:
            ...
            # 验证所有插件都是活跃状态
            for i in range(3):
                assert lifecycle_manager.get_plugin_state(f"plugin_{i}") == PluginState.ACTIVE
            ...
            # 验证所有插件都已停止
            for i in range(3):
                assert lifecycle_manager.get_plugin_state(f"plugin_{i}") == PluginState.STOPPED