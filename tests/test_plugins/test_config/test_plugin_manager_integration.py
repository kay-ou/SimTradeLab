...
class MockTestPluginWithConfigModel(BasePlugin):
    ...
    METADATA = PluginMetadata(
        name="TestPluginWithConfigModel",
        version="1.0.0",
        description="带配置模型的测试插件"
    )
...
class TestPluginManagerConfigValidation:
    ...
    def test_load_plugin_with_wrong_config_type(self):
        """测试使用错误配置类型加载插件"""
        manager = PluginManager()
        ...
        # 使用错误类型的配置对象
        wrong_config = MockDataPluginConfig()
        
        with pytest.raises(PluginLoadError):
            manager.load_plugin(plugin_name, wrong_config)
    ...
    def test_config_conversion_from_pydantic_model(self):
        ...
        base_config = BasePluginConfig(enabled=True)
    ...
    def test_plugin_already_loaded_error(self):
        ...
        # 注册并加载插件
        plugin_name = manager.register_plugin(TestPlugin)
    ...
    def test_batch_load_with_config_validation(self):
        ...
        # 注册多个插件
        plugin1_name = manager.register_plugin(TestPlugin)
        plugin2_name = manager.register_plugin(TestPluginWithConfigModel)
