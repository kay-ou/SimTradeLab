...
class TestQuantitativeTradingPluginSystem:
    ...
    def test_plugin_hot_reload_with_state_preservation(self, plugin_manager):
        ...
        assert self._get_plugin_state(plugin_manager, "stateful_plugin") == PluginState.STARTED
    ...
    def test_plugin_error_isolation_and_system_stability(self, plugin_manager):
        ...
        # 好插件应该正常启动
        assert "mock_data_source" in loaded_plugins
        assert self._get_plugin_state(plugin_manager, "mock_data_source") == PluginState.STARTED
        ...
        assert bad_state != PluginState.STARTED, f"失败的插件不应该处于STARTED状态，实际状态: {bad_state}"
    ...
    def test_concurrent_plugin_operations(self, plugin_manager):
        ...
        # 大部分操作应该成功（允许少量并发冲突）
        assert len(successful_operations) >= 3, f"并发操作成功率过低: {len(successful_operations)}/5"
    ...
    def test_plugin_discovery_and_registration_workflow(self, plugin_manager):
        ...
        for description, plugin_class in registered_plugins:
            ...
            assert self._get_plugin_state(plugin_manager, plugin_name) == PluginState.STARTED, f"{description}未正确启动"
    ...
    def test_plugin_lifecycle_management(self, plugin_manager):
        ...
        assert self._get_plugin_state(plugin_manager, plugin_name) == PluginState.UNINITIALIZED
        
        # 2. 加载和启动阶段
        plugin_manager.load_plugin(plugin_name)
        plugin_manager.start_plugin(plugin_name)
        assert self._get_plugin_state(plugin_manager, plugin_name) == PluginState.STARTED
        ...
        # 4. 停止阶段
        plugin_manager.stop_plugin(plugin_name)
        assert self._get_plugin_state(plugin_manager, plugin_name) == PluginState.STOPPED
