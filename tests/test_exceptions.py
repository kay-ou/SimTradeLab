...
class TestTradingSystemExceptionHandling:
    ...
    def test_invalid_order_parameters_scenario(self):
        ...
        def validate_order_parameters():
            if invalid_price <= 0:
                raise InvalidOrderError(f"Invalid price for {symbol}: {invalid_price}. Price must be positive")
            if invalid_quantity <= 0:
                raise InvalidOrderError(f"Invalid quantity for {symbol}: {invalid_quantity}. Quantity must be positive")
    ...
    def test_data_source_reliability_scenario(self):
        ...
        def fetch_market_data():
            # 模拟网络连接失败
            raise DataSourceError(f"Failed to fetch data for {symbol} from {data_provider}: Connection timeout")
        
        def validate_data_quality():
            # 模拟数据验证失败
            raise DataValidationError(f"Invalid price data for {symbol}: Negative close price detected")
    ...
    def test_configuration_error_in_production_scenario(self):
        ...
        def validate_system_configuration():
            ...
            if not missing_config:
                raise ConfigurationError(f"Required configuration missing: {missing_config}")
            
            # 模拟无效配置值
            risk_threshold = -0.1  # 负值风险阈值
            if risk_threshold < 0:
                raise ConfigurationError(f"Invalid {invalid_config}: {risk_threshold}. Must be non-negative")
...
class TestSystemRecoveryAndDiagnostics:
    ...
    def test_trading_system_error_recovery_flow(self):
        ...
        def simulate_trading_day():
            try:
                ...
                try:
                    # 模拟策略执行失败
                    raise StrategyError("Strategy calculation error due to missing data")
                except StrategyError:
                    system_errors.append("strategy_failure")
                    recovery_actions.append("halted_strategy_execution")
                    
                    # 模拟最终的引擎错误
                    raise EngineError("Trading engine stopped due to multiple component failures")
    ...
    def test_exception_context_preservation_for_debugging(self):
        ...
        def problematic_calculation():
            ...
            try:
                ...
                # 中层策略错误
                try:
                    raise StrategyError(f"Strategy {trading_context['strategy']} failed due to data issues") from data_error
    ...
    def test_production_monitoring_exception_patterns(self):
        ...
        # 资金管理异常
        try:
            raise InsufficientFundsError("Portfolio cash depleted: $125.50 remaining, order requires $50,000")
        ...
        # 数据质量异常
        try:
            raise DataValidationError("Price anomaly detected: 000001.XSHE jumped 150% in 1 second")
        ...
        # 策略性能异常
        try:
            raise StrategyError("Strategy performance degraded: -15% return in last 5 trading days")
        ...
        high_severity_count = sum(1 for p in exception_patterns if p["severity"] == "high")
        immediate_attention_count = sum(1 for p in exception_patterns if p["requires_immediate_attention"])
        
        assert high_severity_count == 1  # 资金管理问题
        assert immediate_attention_count == 1  # 需要立即关注的问题
        
        # 验证具体的业务信息被保留
        fund_pattern = next(p for p in exception_patterns if p["type"] == "fund_management")
