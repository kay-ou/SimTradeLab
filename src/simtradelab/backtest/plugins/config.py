...
class MatchingEngineConfig(BasePluginConfig):
    ...
    price_tolerance: Decimal = Field(
        default=Decimal("0.01"), description="价格容忍度，用于判断限价单是否可以成交", ge=0, le=1
    )

    enable_partial_fill: bool = Field(default=True, description="是否允许部分成交")

    max_order_size: Optional[Decimal] = Field(
        default=None, description="单笔订单最大数量限制", gt=0
    )
...
class SlippageModelConfig(BasePluginConfig):
    ...
    base_slippage_rate: Decimal = Field(
        default=Decimal("0.001"), description="基础滑点率（默认0.1%）", ge=0, le=0.1
    )

    volume_impact_factor: Decimal = Field(
        default=Decimal("0.01"), description="成交量冲击因子", ge=0, le=1
    )

    volatility_multiplier: Decimal = Field(
        default=Decimal("1.0"), description="波动率乘数", ge=0.1, le=10
    )

    min_slippage: Decimal = Field(
        default=Decimal("0.0001"), description="最小滑点（0.01%）", ge=0
    )

    max_slippage: Decimal = Field(
        default=Decimal("0.05"), description="最大滑点（5%）", ge=0, le=0.2
    )
    ...
    def validate_slippage_range(cls, v: Decimal, info) -> Decimal:
        ...
        if (
            hasattr(info, "data")
            and "min_slippage" in info.data
            and v < info.data["min_slippage"]
        ):
...
class CommissionModelConfig(BasePluginConfig):
    ...
    commission_rate: Decimal = Field(
        default=Decimal("0.0003"), description="手续费率（默认万分之3）", ge=0, le=0.01
    )

    min_commission: Decimal = Field(
        default=Decimal("5.0"), description="最低手续费（元）", ge=0
    )

    stamp_duty_rate: Decimal = Field(
        default=Decimal("0.001"), description="印花税率（仅卖出，默认千分之1）", ge=0, le=0.005
    )

    transfer_fee_rate: Decimal = Field(
        default=Decimal("0.00002"), description="过户费率（默认万分之0.2）", ge=0, le=0.001
    )

    enable_stamp_duty: bool = Field(default=True, description="是否启用印花税（A股特有）")

    enable_transfer_fee: bool = Field(default=True, description="是否启用过户费")
...
class DepthMatchingEngineConfig(MatchingEngineConfig):
    ...
    depth_levels: int = Field(default=5, description="考虑的市场深度层级数", ge=1, le=20)

    liquidity_factor: Decimal = Field(
        default=Decimal("0.1"), description="流动性因子，影响大额订单的成交", ge=0.01, le=1
    )

    volume_impact_factor: Decimal = Field(
        default=Decimal("0.001"), description="成交量冲击因子", ge=0, le=0.1
    )

    max_fill_ratio: Decimal = Field(
        default=Decimal("0.1"), description="最大成交比例（相对于市场成交量）", ge=0.01, le=1
    )

    min_fill_amount: Decimal = Field(default=Decimal("100"), description="最小成交金额", ge=0)

    min_depth_ratio: Decimal = Field(
        default=Decimal("0.05"), description="最小深度比例", ge=0, le=1
    )
...
class LimitMatchingEngineConfig(MatchingEngineConfig):
    ...
    strict_limit_check: bool = Field(default=True, description="是否严格检查限价条件")

    order_queue_enabled: bool = Field(default=True, description="是否启用订单队列机制")

    max_queue_size: int = Field(default=1000, description="订单队列最大大小", ge=10, le=10000)

    allow_market_order: bool = Field(default=True, description="是否允许市价单")

    price_precision: int = Field(default=2, description="价格精度（小数位数）", ge=0, le=8)
...
class VolumeBasedSlippageModelConfig(SlippageModelConfig):
    ...
    volume_threshold: Decimal = Field(
        default=Decimal("10000"), description="成交量阈值，超过此值会增加滑点", gt=0
    )
