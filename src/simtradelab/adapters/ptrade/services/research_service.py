...
class ResearchService(BaseService):
    ...
    def __init__(self, context: "PTradeContext", plugin_manager: Optional[Any] = None, **kwargs):
    ...
    def get_history_data(
        ...
        if data_plugin and hasattr(data_plugin, "get_multiple_history_data"):
            try:
                # 使用数据插件获取历史数据
                return data_plugin.get_multiple_history_data(
                ...
                self._logger.warning(f"Data plugin get_history_data failed: {e}")
    ...
    def _calculate_kdj_fallback(
        self, high: np.ndarray, low: np.ndarray, close: np.ndarray, n: int, m1: int, m2: int
    ) -> pd.DataFrame:
        """KDJ指标计算后备实现"""
        length = len(close)
        return pd.DataFrame({
