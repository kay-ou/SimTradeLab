...
class TradingService(BaseService):
    ...
    def execute_order_target(
        ...
        if trade_amount == 0:
            self._logger.info(
                f"No trade needed for {security}: already at target amount"
            )
            return None
    ...
    def set_parameters(self, params: Dict[str, Any]) -> None:
        ...
        self.publish_event(
            "live_trading.parameters.updated", {"parameters": params}, "trading_service"
        )
    ...
    def handle_data_update(self, data: Dict[str, Any]) -> None:
        ...
        # 发布数据更新事件
        self.publish_event(
            "live_trading.data.updated", {"data": data}, "trading_service"
        )
    ...
    def _get_market_price(self, security: str) -> float:
        ...
        self._logger.warning(
            f"Using default price for {security} - should connect to real market data"
        )
        return 10.0
