...
class BacktestService(BaseService):
    ...
    def set_limit_mode(self, mode: str) -> None:
        ...
        if mode not in valid_modes:
            raise ValueError(f"Invalid limit mode: {mode}. Must be one of {valid_modes}")
    ...
    def cancel_order(self, order_id: str) -> bool:
        ...
        if order.status != "submitted":
            self._logger.warning(f"Cannot cancel order {order_id} with status {order.status}")
            return False
    ...
    def _update_position(
        ...
        if security in self.context.portfolio.positions:
            ...
            if amount > 0:  # 买入：更新成本基础
                ...
                position.cost_basis = total_cost / total_amount if total_amount > 0 else 0
        ...
        for order in orders:
            if order.status == "filled":
                trades.append({
                    ...
                    "commission": abs(order.filled * (order.limit or 0.0) * self._commission_rate),
                    "datetime": order.dt,
