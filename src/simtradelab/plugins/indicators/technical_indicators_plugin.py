...
class TechnicalIndicatorsPlugin(BasePlugin):
    ...
    def __init__(self, metadata: PluginMetadata, config: Optional[PluginConfig] = None):
        ...
        self.macd_params = self._config.config.get('macd', {'short': 12, 'long': 26, 'm': 9})
    ...
    def _on_initialize(self) -> None:
        ...
        self._logger.info("Technical Indicators Plugin initialized with indicators: "
                         f"MACD, KDJ, RSI, CCI")
    ...
    def calculate_kdj(self, high: np.ndarray, low: np.ndarray, close: np.ndarray) -> pd.DataFrame:
        """计算KDJ指标"""
    ...
    def calculate_cci(self, high: np.ndarray, low: np.ndarray, close: np.ndarray) -> pd.DataFrame:
        """计算CCI指标"""
        n = self.cci_params['n']
