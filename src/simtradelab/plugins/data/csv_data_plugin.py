...
class CSVDataPlugin(BasePlugin):
    ...
    def __init__(self, metadata: PluginMetadata, config: Optional[PluginConfig] = None):
        ...
        # 数据存储目录
        self._data_dir = Path(self._config.config.get('data_dir', 
    ...
    def _on_initialize(self) -> None:
        ...
        self._logger.info(f"CSV Data Plugin initialized with data directory: {self._data_dir}")
    
    def _ensure_base_data_files(self) -> None:
        """确保基础数据文件存在"""
        # 创建常用股票的历史数据文件
        common_stocks = [
            "000001.SZ", "000002.SZ", "000858.SZ", "600000.SH", 
            "600036.SH", "600519.SH", "688001.SH", "300001.SZ"
        ]
    ...
    def get_history_data(self, security: str, count: int, 
        ...
        if cache_key in self._data_cache:
            ...
            if cache_time and (datetime.now() - cache_time).seconds < self._cache_timeout:
    ...
    def get_multiple_history_data(self, securities: List[str], count: int,
                                 start_date: Optional[str] = None,
                                 end_date: Optional[str] = None) -> pd.DataFrame:
        """
        获取多个证券的历史数据
            ...
            try:
                ...
                if not df.empty:
                    ...
                    snapshot[security] = {
                        ...
                        'pre_close': float(latest['close']) * (1 + np.random.normal(0, 0.001)),
    ...
    def _clear_cache_for_security(self, security: str) -> None:
        """清除指定证券的缓存"""
        keys_to_remove = [key for key in self._data_cache.keys() if key.startswith(security)]
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'cache_size_mb': sum(df.memory_usage(deep=True).sum() for df in self._data_cache.values()) / (1024 * 1024),
            'cache_timeout': self._cache_timeout,
            'oldest_cache_time': min(self._cache_timestamps.values()) if self._cache_timestamps else None
        }
