            ...
            for field in fields:
                if table == "income":  # 利润表
                    if field in ["revenue", "total_revenue"]:  # 营业收入
                        ...
                        row[field] = random.uniform(
                            200000, 1500000
                        )  # nosec B311  # 200K-1.5M
                    elif field in ["total_profit"]:  # 利润总额
                        row[field] = random.uniform(
                            150000, 1200000
                        )  # nosec B311  # 150K-1.2M
                    elif field in ["operating_cost"]:  # 营业成本
                        row[field] = random.uniform(
                            500000, 8000000
                        )  # nosec B311  # 500K-8M
                    else:
                        row[field] = random.uniform(0, 500000)  # nosec B311
                    ...
                    if field in ["total_assets"]:  # 总资产
                        ...
                        row[field] = random.uniform(0, 10000000)  # nosec B311
                    ...
                    if field in ["operating_cash_flow"]:  # 经营现金流
                        ...
                        row[field] = random.uniform(-500000, 500000)  # nosec B311

                else:  # 其他表格或比率数据
                    if field in ["pe_ratio"]:  # 市盈率
                        row[field] = random.uniform(10, 50)  # nosec B311
                    elif field in ["pb_ratio"]:  # 市净率
                        row[field] = random.uniform(1, 10)  # nosec B311
                    elif field in ["roe"]:  # 净资产收益率
                        row[field] = random.uniform(0.05, 0.25)  # nosec B311
                    elif field in ["eps"]:  # 每股收益
                        row[field] = random.uniform(0.1, 5.0)  # nosec B311
                    elif field in ["bps"]:  # 每股净资产
                        row[field] = random.uniform(3.0, 15.0)  # nosec B311
                    elif field in ["debt_ratio"]:  # 资产负债率
                        row[field] = random.uniform(0.3, 0.8)  # nosec B311
                    elif field in ["current_ratio"]:  # 流动比率
                        row[field] = random.uniform(1.0, 3.0)  # nosec B311
                    elif field in ["quick_ratio"]:  # 速动比率
                        row[field] = random.uniform(0.5, 2.0)  # nosec B311
                    else:
                        row[field] = random.uniform(0, 1000000)  # nosec B311
