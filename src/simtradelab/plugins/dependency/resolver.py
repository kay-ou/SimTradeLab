...
class DependencyResolver:
    ...
    def find_update_path(
        self, current_plugins: List[str], target_plugins: List[str]
    ) -> Dict[str, str]:
        """
        找到从当前插件配置到目标配置的更新路径
        ...
        if to_remove:
            ...
            try:
                ...
                update_plan["sequence"].extend(
                    [("remove", p) for p in reversed(remove_order)]
                )
            except Exception:
                # 如果无法解析，按原序列卸载
    ...
    def _collect_all_dependencies(
        self, plugins: List[str], collected: Optional[Set[str]] = None
    ) -> Set[str]:
        """递归收集所有依赖的插件"""
