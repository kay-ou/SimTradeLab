...
from .manifest import ManifestValidator, PluginCategory, PluginManifest
...
class PluginRegistry:
    ...
    def search_plugins(
        ...
        if keyword:
            ...
            for plugin_name in candidates:
                ...
                if (
                    keyword_lower in manifest.name.lower()
                    or keyword_lower in manifest.description.lower()
                    or any(keyword_lower in kw.lower() for kw in manifest.keywords)
                ):
    ...
    def get_statistics(self) -> Dict[str, any]:
        ...
        stats = {
            ...
            "by_category": {
                cat.value: len(plugins) for cat, plugins in self._by_category.items()
            },
            "by_author": {
                author: len(plugins) for author, plugins in self._by_author.items()
            },
            "top_tags": [],
        ...
        try:
            ...
            def convert_objects_to_serializable(obj):
                if isinstance(obj, datetime):
                    ...
                    return {
                        k: convert_objects_to_serializable(v) for k, v in obj.items()
                    }
