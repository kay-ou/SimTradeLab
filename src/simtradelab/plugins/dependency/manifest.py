...
class PluginManifest(BaseModel):
    ...
    dependencies: Dict[str, str] = Field(
        ...
        description="插件依赖 {plugin_name: version_spec}"
    )
    ...
    # 安全和资源
    security_level: SecurityLevel = Field(default=SecurityLevel.MEDIUM, description="安全等级")
...
class ManifestValidator:
    ...
    @staticmethod
    def save_to_file(manifest: PluginManifest, manifest_path: Path, format: str = "yaml") -> None:
        """保存插件清单到文件"""
    ...
    def validate_manifest(manifest: PluginManifest) -> List[str]:
        ...
        # 检查安全等级和资源的匹配性
        if manifest.security_level == SecurityLevel.LOW and manifest.max_memory_mb > 256:
            errors.append("低安全等级插件的内存使用不应超过256MB")
        
        if manifest.security_level == SecurityLevel.CRITICAL and manifest.max_cpu_percent > 80:
