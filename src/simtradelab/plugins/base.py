...
class BasePlugin(abc.ABC):
        ...
        try:
            ...
            with self._lock:
                ...
                try:
                    ...
                    self._logger.info(
                        f"Configuration updated for plugin {self._metadata.name}"
                    )
        ...
        with self._lock:
            if self._state != PluginState.UNINITIALIZED:
                raise PluginStateError(
                    f"Plugin {self._metadata.name} is already initialized"
                )
            ...
            try:
                ...
                self._logger.info(
                    f"Plugin {self._metadata.name} initialized successfully"
                )
                ...
                self._error_count += 1
                self._logger.error(
                    f"Failed to initialize plugin {self._metadata.name}: {e}"
                )
        ...
        with self._lock:
            ...
            if not self._config.enabled:
                self._logger.warning(
                    f"Plugin {self._metadata.name} is disabled, skipping start"
                )
                return
    ...
    def stop(self) -> None:
        ...
        with self._lock:
            if self._state not in [PluginState.STARTED, PluginState.PAUSED]:
                self._logger.warning(
                    f"Plugin {self._metadata.name} is not running, state: {self._state}"
                )
                return
    ...
    def resume(self) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                self._error_count += 1
                self._logger.error(
                    f"Failed to resume plugin {self._metadata.name}: {e}"
                )
    ...
    def _on_mode_changed(
        self, old_mode: Optional[Enum], new_mode: Optional[Enum]
    ) -> None:
        """
        模式变更时调用的钩子方法
        子类可以重写此方法来处理模式变更
