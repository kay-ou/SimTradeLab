...
class EventSubscription:
    ...
    subscription_id: str = field(default_factory=lambda: f"sub_{time.time()}_{id(object())}")
    ...
    def subscribe(
        ...
        try:
            with self._lock:
                ...
                self._logger.debug(f"Subscribed to {event_type} with ID {subscription.subscription_id}")
    ...
    def unsubscribe_all(self, event_type: str) -> int:
        ...
        with self._lock:
            if event_type in self._subscriptions:
                ...
                self._logger.debug(f"Unsubscribed all {count} handlers from {event_type}")
                return count
            return 0
        ...
        for subscription in subscriptions:
            try:
                ...
                if subscription.async_handler:
                    # 异步处理函数在同步模式下跳过
                    self._logger.warning(f"Skipping async handler for {event.type} in sync mode")
                    continue
                else:
        ...
        if tasks:
            ...
            for result in async_results:
                if isinstance(result, Exception):
                    self._stats['events_failed'] += 1
                    self._logger.error(f"Async handler failed for event {event.type}: {result}")
                    results.append(None)
                else:
                    results.append(result)
                    self._stats['events_processed'] += 1
