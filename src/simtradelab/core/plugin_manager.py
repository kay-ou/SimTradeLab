...
class PluginManager:
    ...
    def register_plugin(
        ...
        if not issubclass(plugin_class, BasePlugin):
            raise PluginRegistrationError(f"{plugin_class} is not a BasePlugin subclass")
        ...
        try:
            ...
            if hasattr(plugin_class, 'METADATA'):
                ...
                metadata = PluginMetadata(
                    ...
                    version="1.0.0",
                    description=plugin_class.__doc__ or f"Plugin {plugin_class.__name__}"
                )
            ...
            with self._lock:
                ...
                if plugin_name in self._plugins:
                    raise PluginRegistrationError(f"Plugin {plugin_name} is already registered")
            ...
            raise PluginRegistrationError(f"Failed to register plugin {plugin_class}: {e}")
    ...
    def load_plugin(self, plugin_name: str, config: Optional[PluginConfig] = None) -> BasePlugin:
        """
        加载插件
    ...
    def get_plugin_info(self, plugin_name: str) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            info = {
                ...
                'state': registry.instance.state.value if registry.instance else 'unloaded'
            }
            ...
            for pkg_dir in directory.iterdir():
                if pkg_dir.is_dir() and (pkg_dir / "__init__.py").exists():
                    try:
                        ...
                        self._logger.warning(f"Failed to load plugin from {pkg_dir}: {e}")
